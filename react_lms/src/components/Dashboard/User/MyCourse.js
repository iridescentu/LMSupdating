...
import { apiGetCompletedContentHistories, apiGetContentHistoriesByCourse, apiGetMyContentHistory, apiGetMyCourseHistroies } from '../../RestApi';
    ...
    <>
      <Container>
        {courseHistoryDtos.map((courseHistoryDto) => (
          <div key={courseHistoryDto.courseHistory.courseHistoryId} className='courseBox'>
            <StyledNavLink
              to={`/course/${courseHistoryDto.courseHistory.course.courseId}`}
              style={{ textDecoration: 'none' }}
            >
              <h2>[{courseHistoryDto.courseHistory.course.subject.subjectName}] {courseHistoryDto.courseHistory.course.courseName}</h2>
            </StyledNavLink>
            <p>
              Start Date:{' '}
            ...
            <p>
              총 {courseHistoryDto.totalContents} 강, {courseHistoryDto.courseHistory.course.durationMins}분
            </p>

            <p>수료증: {courseHistoryDto.courseHistory.contentStatus ? "발급 완료" : "미수료" }</p>
            <StyledNavLink
